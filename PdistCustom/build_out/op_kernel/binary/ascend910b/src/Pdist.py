...
def pdist(x_in__, y_out_, p, kernel_name="pdist", impl_mode=""):
    ...
    msg = "start compile Acend C Operator Pdist, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Pdist"
...
def op_select_format(x_in__, y_out_, p, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p)
    result = check_op_cap("op_select_format", "Pdist", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, p, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, p)
